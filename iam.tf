...
resource "aws_iam_group_membership" "jenkins-administrators-users-assignment" {
  name = "jekins-administrators-users"
...
resource "aws_iam_group_membership" "jenkins-users-assignment" {
  name = "jekins-users"
  users = [aws_iam_user.jenkins-dev-team.id,aws_iam_user.jenkins-test-team.id]
  group = aws_iam_group.jenkins-users.id
}

# Attaches a Managed IAM Policy to user(s), role(s), and/or group(s)-https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy_attachment
